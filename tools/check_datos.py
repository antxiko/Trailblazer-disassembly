...
mentiras. En Stardust colo tres veces.

Lo que hace: para cada rango `D` de cada listado, mira cuantos de sus bytes caen
en bloques que el trazado marca como codigo, y avisa. Un solapamiento pequeno
puede ser legitimo -una tabla que empieza justo donde acaba una rutina-, asi que
se ordena por gravedad y se da el porcentaje, que es lo que distingue un byte de
borde de una zona entera mal leida.

Uso:  check_datos.py <dir_work> <dir_src> [--umbral=N]
...
def main(work, src, *resto):
    ...
    print("Cruzando %d zonas de datos declaradas contra el trazado." % total)
    if not sospechosas:
